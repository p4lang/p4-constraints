// Copied from https://github.com/google/nucleus.
...
// where:
//
//   - pb can be either a protobuf value or a human-readable std::string
//     representation of it.
//   - When pb is a std::string, the matcher can optionally accept a
...
//                     list of std::strings and each element is specified by
//                     their fully qualified names, i.e., the names
//                     corresponding to FieldDescriptor.full_name().  m can be
...
//                     container or an initializer list of std::strings and
...
class ProtoMatcherBase {
 public:
  ...
  // Makes this matcher ignore std::string elements specified by their fully
  ...
  // Makes this matcher ignore std::string elements specified by their relative
  // FieldPath.
  template <class Iterator>
...
// Implements EqualsProto, EquivToProto, EqualsInitializedProto, and
// EquivToInitializedProto, where the matcher parameter is a std::string.
class ProtoStringMatcher : public ProtoMatcherBase {
 public:
  ...
  virtual const google::protobuf::Message* CreateExpectedProto(
    ...
    // We don't insist that the expected std::string parses as an
...
class WhenDeserializedMatcherBase {
 public:
  ...
  bool MatchAndExplain(google::protobuf::io::ZeroCopyInputStream* arg,
                       ::testing::MatchResultListener* listener) const {
    // Deserializes the std::string arg as a protobuf of the same type as the
    // expected protobuf.
...
// WhenDeserialized(m) is a matcher that matches a std::string that can be
...
// WhenDeserializedAs<Proto>(m) is a matcher that matches a std::string
