...
inline ::util::Status OkStatus() { return absl::OkStatus(); }

extern std::string* MediaPipeCheckOpHelperOutOfLine(
    const ::util::Status& v, const char* msg);

inline std::string* MediaPipeCheckOpHelper(::util::Status v,
                                           const char* msg) {
...
#define MEDIAPIPE_DO_CHECK_OK(val, level)                               \
  while (auto _result = ::util::MediaPipeCheckOpHelper(val, #val)) \
  LOG(level) << *(_result)
...
// MEDIAPIPE_ prefix in util's codebase.
...
#define MEDIAPIPE_DCHECK_OK(val) \
  while (false && (::util::OkStatus() == (val))) LOG(FATAL)
#endif
